...
- Éxito: `201` con
  `{ id, email, role: "requester", createdAt }` — sin hash ni secretos.
- **Errores:**
  ...
  - `409 ACCOUNT_CANNOT_BE_CREATED` — email ya registrado (genérico, no filtra cuál). El `23505` de la BD se traduce a este código genérico.
...
- Éxito: `200` con `{ id, email, role }` del token autenticado.
- **Errores:**
...
- `403` = autorización: estás autenticado pero tu rol/propiedad no lo permite
  (`FORBIDDEN`). Ejemplo: un requester que intenta cambiar prioridad o estado.
...
- `409` = conflicto de dominio: transición de estado inválida
  (`INVALID_STATUS_TRANSITION`), estado terminal immutable
  (`REQUEST_IN_TERMINAL_STATUS`), email duplicado (`ACCOUNT_CANNOT_BE_CREATED`).
